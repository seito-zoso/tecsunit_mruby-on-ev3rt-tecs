 ...
 * ARG_NAME_LEN     uint8_t          ATTR_ARG_NAME_LEN
 * ARG_DIM          uint8_t          ATTR_ARG_DIM
 * TARGET_NUM       uint8_t          ATTR_TARGET_NUM
...
eBody_main(CELLIDX idx)
{
  ...
  strcpy( VAR_cell_path ,"rDomainEV3::Target1" );

  cLCD_setFont( EV3_FONT_MEDIUM );
  cLCD_drawString( "TECSInfo test", 1, 0 );
  cLCD_drawString( "path:", 1, 1 );
  cLCD_drawString( VAR_cell_path, 7, 1 );
  while(1){
