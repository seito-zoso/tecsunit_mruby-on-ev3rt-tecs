...
eInitialize_initializeBridge(CELLIDX idx, mrb_state* mrb, struct RClass* TECS)
{
    ...
    mrb_define_method( mrb, rc, "initialize", MrubyBridge_tsSpeaker_initialize, MRB_ARGS_REQ(1) );
    MRB_SET_INSTANCE_TT(rc, MRB_TT_DATA);
	mrb_define_method( mrb, rc, "setVolume", MrubyBridge_tsSpeaker_setVolume, MRB_ARGS_REQ( 1 ) );
	mrb_define_method( mrb, rc, "playTone", MrubyBridge_tsSpeaker_playTone, MRB_ARGS_REQ( 2 ) );
	mrb_define_method( mrb, rc, "stop", MrubyBridge_tsSpeaker_stop, MRB_ARGS_NONE() );
}
