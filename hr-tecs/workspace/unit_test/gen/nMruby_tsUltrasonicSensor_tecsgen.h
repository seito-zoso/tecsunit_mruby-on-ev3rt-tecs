...
#define nMruby_tsUltrasonicSensor_CB_ptab           nMruby_tsUltrasonicSensor_INIB_ptab
...
extern nMruby_tsUltrasonicSensor_INIB  *const nMruby_tsUltrasonicSensor_INIB_ptab[];
...
/* セルCBを得るマクロ #_GCB_# */
#define nMruby_tsUltrasonicSensor_GET_CELLCB(idx) (nMruby_tsUltrasonicSensor_CB_ptab[(idx) - nMruby_tsUltrasonicSensor_ID_BASE])
#ifndef TECSFLOW
 /* 呼び口関数マクロ #_CPM_# */
...
#else  /* TECSFLOW */
#define nMruby_tsUltrasonicSensor_cTECS_getDistance( p_that ) \
	  (p_that)->cTECS.getDistance__T( \
 )
#define nMruby_tsUltrasonicSensor_cTECS_listen( p_that ) \
	  (p_that)->cTECS.listen__T( \
 )
#define nMruby_tsUltrasonicSensor_cTECS_initializePort( p_that ) \
...
#define FOREACH_CELL(i,p_cb)   \
    for( (i) = 0; (i) < nMruby_tsUltrasonicSensor_N_CELL; (i)++ ){ \
       (p_cb) = nMruby_tsUltrasonicSensor_CB_ptab[i];
